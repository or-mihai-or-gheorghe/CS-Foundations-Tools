# tools/games/speed_hex_conversion.py
...
GAME_SLUG = "speed_hex_conversion"
GAME_DISPLAY_NAME = "Speed Hex Conversion"

# ========================= Hex Conversion Utilities =========================

def decimal_to_hex(val: int) -> str:
    """Convert decimal to hex (uppercase, no 0x prefix)"""
    return hex(val)[2:].upper()

def hex_to_decimal(hex_str: str) -> int:
    """Convert hex to decimal (case-insensitive)"""
    return int(hex_str, 16)

def generate_hex_distractors(correct_hex: str, decimal_val: int, count: int = 3) -> List[str]:
    """Generate plausible wrong hex answers"""
    distractors = set()

    # Strategy 1: Off-by-one in decimal
    if decimal_val > 0:
        distractors.add(hex(decimal_val - 1)[2:].upper())
    distractors.add(hex(decimal_val + 1)[2:].upper())

    # Strategy 2: Swap hex digits (if multi-digit)
    if len(correct_hex) > 1:
        swapped = correct_hex[-1] + correct_hex[1:-1] + correct_hex[0] if len(correct_hex) > 2 else correct_hex[::-1]
        distractors.add(swapped)

    # Strategy 3: Flip bits in decimal then convert
    bit_length = decimal_val.bit_length()
    for _ in range(3):
        if bit_length > 0:
            flipped = decimal_val ^ (1 << random.randint(0, bit_length - 1))
            distractors.add(hex(flipped)[2:].upper())

    # Strategy 4: Random nearby values
    for offset in [-2, 2, -3, 3]:
        candidate = decimal_val + offset
        if candidate > 0:
            distractors.add(hex(candidate)[2:].upper())

    # Remove correct answer
    distractors.discard(correct_hex)
    distractors = {d for d in distractors if d}

    # Ensure we have enough
    while len(distractors) < count:
        noise = random.randint(-5, 5)
        candidate = max(1, decimal_val + noise)
        if candidate != decimal_val:
            distractors.add(hex(candidate)[2:].upper())

    return random.sample(list(distractors), min(count, len(distractors)))

def generate_binary_distractors(correct_bin: str, decimal_val: int, count: int = 3) -> List[str]:
    """Generate plausible wrong binary answers"""
    distractors = set()
    bit_length = len(correct_bin)

    # Strategy 1: Off-by-one in decimal
    if decimal_val > 0:
        distractors.add(bin(decimal_val - 1)[2:])
    distractors.add(bin(decimal_val + 1)[2:])

    # Strategy 2: Flip random bits
    for _ in range(5):
        flipped = decimal_val ^ (1 << random.randint(0, bit_length - 1))
        distractors.add(bin(flipped)[2:])

    # Strategy 3: Wrong bit length (add/remove leading bits)
    if len(correct_bin) > 1:
        distractors.add(correct_bin[1:])  # Remove leading bit
    distractors.add('1' + correct_bin)  # Add leading bit

    # Remove correct answer
    distractors.discard(correct_bin)
    distractors = {d for d in distractors if d}
...
def init_game_state():
    ...
    if 'hex_game' not in st.session_state:
        st.session_state.hex_game = {
            'active': False,
            'mode': None,
            'difficulty': None,
            'input_type': None,
...
def is_game_active() -> bool:
    ...
    game = st.session_state.hex_game
...
def generate_question():
    """Generate a new hex conversion question"""
    game = st.session_state.hex_game
    ...
    if game['mode'] == 'Mixed':
        # Alternate between Binary→Hex and Hex→Binary
        question_type = 'binary_to_hex' if game['total_count'] % 2 == 0 else 'hex_to_binary'
    elif game['mode'] == 'Binary → Hex':
        question_type = 'binary_to_hex'
    else:  # Hex → Binary
        question_type = 'hex_to_binary'

    if question_type == 'binary_to_hex':
        question = {
            'type': 'binary_to_hex',
            'question': binary_str,
            'answer': hex_str,
            'display_question': f"Convert: `{binary_str}` to hex",
            'choices': None
        }

        if game['input_type'] == 'Multiple Choice':
            distractors = generate_hex_distractors(hex_str, decimal_val, 3)
            choices = [hex_str] + distractors
        ...
        question = {
            ...
            'question': hex_str,
            'answer': binary_str,
            'display_question': f"Convert: `{hex_str}` to binary",
            'choices': None
        }

        if game['input_type'] == 'Multiple Choice':
            distractors = generate_binary_distractors(binary_str, decimal_val, 3)
...
def check_answer(user_answer: str, is_skip: bool = False) -> bool:
    ...
    # Normalize answers for comparison (case-insensitive for hex)
...
def render_setup_screen():
    ...
    st.markdown("""
    ### How to Play
    Convert between binary and hexadecimal as fast as you can in **60 seconds**!
...
def render_game_screen():
    ...
    if game['input_type'] == 'Multiple Choice':
        ...
        with st.form(key='answer_form', clear_on_submit=True):
            if question['type'] == 'binary_to_hex':
                user_answer = st.text_input("Enter hexadecimal (e.g., A5, FF):", key='answer_input')
            else:
                user_answer = st.text_input("Enter binary (e.g., 10110):", key='answer_input')
